...
const SessionPage = () => {
  ...
  const [bookingData, setBookings] = useState<IBookingData[]>([]);
    ...
    const fetchSessions = async () => {
      ...
      try {
        ...
        const SRes = await fetch(
          `${process.env.NEXT_PUBLIC_BACKKEND_URL}/api/v1/tutoring-sessions/sessions/all`
        );
        ...
        // Stop loading only if we are not waiting for booking data
        if (!user?.id) {
          setIsLoading(false);
        }
