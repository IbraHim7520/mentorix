...
const SessionCreateForm = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_BACKKEND_URL}/api/v1/categories/category/all-category/general`, { credentials: 'include' });
  ...
  const handleSessionFeeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(Number(value)) && Number(value) >= 0) setSessionFee(value);
  };
  ...
  const onSubmit = async (data: ISessionCreate) => {
    ...
    setLoading(true);
    data.sessionFee = parseFloat(sessionFee) || 0;
    data.tutorId = user.id;

    if (!data.categoryId) return toast.error("Please select a category!");
    if (!data.sessionFee) return toast.error("Session Fee is required!");

    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKKEND_URL}/api/v1/tutoring-sessions/session-create`, {
        method: 'POST',
      ...
      if (result?.success) {
        reset();
        setSessionFee("");
        toast.success(result.message || "Session published successfully");
      } else {
        toast.error(result?.message || "Failed to publish session");
      }
    } catch {
    ...
    <div className="max-w-full mx-auto bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-8">
        ...
        <div className="pt-6 flex flex-col sm:flex-row gap-4">
          ...
          >
            {loading ? <Spinner className="size-6" /> : " Publish Session"}
          </button>
          <button
